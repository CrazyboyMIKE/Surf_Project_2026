...
11. [ ] Web user can request controller ownership.
12. [ ] Viewer robot-control attempt is rejected.
13. [ ] `1000 stop` test passes.
14. [ ] `1002` test passes if real robot movement is connected in a later round; otherwise confirm mock log/display only.
15. [ ] `1003` test passes if real robot movement is connected in a later round; otherwise confirm mock log/display only.
16. [ ] Android true device joins the room.
17. [ ] Android true device publishes camera.
18. [ ] Phone 4G/5G can watch Web robot video.
19. [ ] LiveKit Cloud dashboard usage is checked.
20. [ ] Issues are recorded with timestamp, room name, participant type, browser/device, and logs without secrets.
